/**
 * POST /api/bitrix24/chat/recent для Express (Node).
 * Логика продублирована из api/bitrix24/chat/recent.ts — без импортов из api/.
 */

export type Bitrix24ChatRecentHttpResult = {
  status: number;
...
export type Bitrix24RecentChatOut = {
  dialogId: string;
  chatId?: number;
  title: string;
...
export async function runBitrix24ChatRecent(_rawBody: unknown): Promise<Bitrix24ChatRecentHttpResult> {
  ...
  try {
    ...
    try {
      ...
      console.error("[bitrix24] chat/recent bitrix non-json");
      return {
        status: 502,
        body: {
          success: false,
    ...
    console.error("[bitrix24] chat/recent bitrix network", m);
    return {
      status: 502,
      body: {
        success: false,
  ...
  if (bitrixJson.error) {
    ...
    console.error("[bitrix24] chat/recent bitrix api error", { bitrixCode });
    return {
      status: 502,
      body: {
        success: false,
