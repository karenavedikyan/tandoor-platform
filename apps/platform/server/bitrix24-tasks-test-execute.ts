/**
 * Общая логика POST /api/bitrix24/tasks/test для Express (Node) и Vercel Serverless.
 * Не импортирует Express; не логирует webhook URL.
 */
...
export async function runBitrix24TasksTest(): Promise<Bitrix24TasksTestHttpResult> {
  ...
  if (bitrixJson.error) {
    ...
    console.error("[bitrix24] Bitrix24 API error", { bitrixCode });
    return {
      status: 502,
      body: {
        success: false,
