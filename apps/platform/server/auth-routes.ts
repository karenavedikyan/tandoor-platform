...
export function registerAuthRoutes(app: Express): void {
  ...
  app.get("/api/auth/my-org-snapshot", requireAuth(), async (req: Request, res: Response) => {
    try {
      if (!req.auth) {
        res.status(401).json({ success: false, code: "UNAUTHORIZED" });
        return;
      }
      const result = await myOrgSnapshotHandler({ auth: req.auth });
      applyAuthHttpResult(res, result);
    } catch (e) {
      const m = e instanceof Error ? e.message : String(e);
      console.error("[api/auth] my-org-snapshot", m.slice(0, 200));
      res.status(500).json({
        success: false,
        code: "INTERNAL_ERROR",
        message: "Внутренняя ошибка сервера.",
