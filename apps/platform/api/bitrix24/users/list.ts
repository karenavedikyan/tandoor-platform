/**
 * Vercel Serverless: POST /api/bitrix24/users/list
 *
...
/** При поиске: максимум страниц user.get, чтобы не уйти в бесконечный обход портала. */
...
async function fetchUserGetPage(
  ...
  try {
    ...
    try {
      ...
      console.error("[bitrix24-api] users/list bitrix non-json", "http", bxRes.status);
      return { ok: false, kind: "bad_json" };
    }
  } catch (e) {
    const m = e instanceof Error ? e.message : "unknown";
    console.error("[bitrix24-api] users/list bitrix network", m);
    return { ok: false, kind: "network" };
  }

  if (bitrixJson.error) {
    const bitrixCode = typeof bitrixJson.error === "string" ? bitrixJson.error : "UNKNOWN";
    console.error("[bitrix24-api] users/list bitrix api error", { bitrixCode });
...
async function runBitrix24UsersListCore(rawBody: unknown): Promise<{ status: number; body: Record<string, unknown> }> {
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  try {
    ...
    const out = await runBitrix24UsersListCore(body ?? {});
    sendJson(res, out.status, out.body);
  } catch (e) {
    const m = e instanceof Error ? e.message : String(e);
    console.error("[bitrix24-api] users/list unhandled", m);
    sendJson(res, 500, {
      success: false,
      code: "INTERNAL_ERROR",
