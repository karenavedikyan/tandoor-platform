/**
 * Vercel Serverless: POST /api/bitrix24/tasks/list
 *
...
/**
 * Опциональный responsibleId из тела запроса: положительное целое (строка или число).
 * Если поле отсутствует или null — используется серверный fallback (env / URL webhook).
 */
...
async function runBitrix24TasksListCore(rawBody: unknown): Promise<{ status: number; body: Record<string, unknown> }> {
  ...
  try {
    ...
    try {
      ...
      console.error("[bitrix24-api] tasks/list bitrix non-json", "http", bxRes.status);
      return {
        status: 502,
        body: {
          success: false,
    ...
    console.error("[bitrix24-api] tasks/list bitrix network", m);
    return {
      status: 502,
      body: {
        success: false,
  ...
  if (bitrixJson.error) {
    ...
    console.error("[bitrix24-api] tasks/list bitrix api error", { bitrixCode });
    return {
      status: 502,
      body: {
        success: false,
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  try {
    ...
    const out = await runBitrix24TasksListCore(body ?? {});
    sendJson(res, out.status, out.body);
  } catch (e) {
    const m = e instanceof Error ? e.message : String(e);
    console.error("[bitrix24-api] tasks/list unhandled", m);
    sendJson(res, 500, {
      success: false,
      code: "INTERNAL_ERROR",
