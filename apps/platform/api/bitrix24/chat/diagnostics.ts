/**
 * Vercel Serverless: POST /api/bitrix24/chat/diagnostics
 *
 * Полностью автономный handler: без импортов из других файлов api/, server/*, client/*, @/.
 * Диагностика REST im.* через webhook; ошибки по отдельным методам не роняют весь ответ.
 */
...
function entryFromBitrixCall(
  ...
  console.error("[bitrix24-api] chat/diagnostics bitrix error", { method, bitrixCode: out.code });
  return {
    method,
    success: false,
    bitrixCode: out.code,
...
async function runChatDiagnosticsCore(rawBody: unknown): Promise<{ status: number; body: Record<string, unknown> }> {
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  try {
    ...
    const out = await runChatDiagnosticsCore(body ?? {});
    sendJson(res, out.status, out.body);
  } catch (e) {
    const m = e instanceof Error ? e.message : String(e);
    console.error("[bitrix24-api] chat/diagnostics unhandled", m);
    sendJson(res, 500, {
      success: false,
      code: "INTERNAL_ERROR",
