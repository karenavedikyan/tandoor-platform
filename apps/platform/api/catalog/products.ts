  ...
  try {
    ...
    const r = await pool.query<{
      ...
      `SELECT
         ...
         p.brand,
         p.is_on_site,
         (SELECT i.path FROM catalog_product_images i WHERE i.product_id = p.id ORDER BY i.sort_order ASC NULLS LAST, i.path ASC LIMIT 1) AS image_path,
         (SELECT i.blob_url FROM catalog_product_images i WHERE i.product_id = p.id AND i.blob_url IS NOT NULL ORDER BY i.sort_order ASC NULLS LAST, i.path ASC LIMIT 1) AS image_url,
