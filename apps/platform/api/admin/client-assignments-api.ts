/**
 * Эндпоинты сидинга команд/клиентов и перепривязки (Prompt 18).
 * Импортируется из `api/admin/[action].ts`.
 */
...
export async function handleTeamsSeed(
  res: VercelResponse,
  pool: PoolLike,
  me: DbUserRow,
): Promise<void> {
  ...
  for (const su of SALES_USERS) {
    ...
    const rc = ins.rowCount;
    if (typeof rc === "number" && rc > 0) membershipsInserted += rc;
  }
...
export async function handleClientsAssignmentsSeed(
  res: VercelResponse,
  pool: PoolLike,
  me: DbUserRow,
): Promise<void> {
