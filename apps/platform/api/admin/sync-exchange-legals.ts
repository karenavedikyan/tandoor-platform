/**
 * POST /api/admin/sync-exchange-legals — триггер импорта users1.xml на VM runner (streaming FTP).
 */
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  try {
    ...
    const r = await fetch(`${proxy.proxyUrl}/run/legals`, {
      method: "POST",
      headers,
    ...
    sendJson(res, 202, {
      ...
      message: "Импорт контрагентов 1С запущен на VM (streaming, может занять несколько минут)",
      runner: json,
    });
  } catch (e) {
    const m = e instanceof Error ? e.message : String(e);
    console.error("[sync-exchange-legals]", m);
