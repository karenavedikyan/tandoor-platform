...
- **Список клиентов:** `pages/dealer-base.tsx` — строки из merge, кнопка «Добавить клиента», синхронизация. Поиск (`lib/dealer-base-picker-filters.ts`) ищет по названию, коду, городу, РОП, менеджеру, типу, адресу, идентификатору и `actualizationInn` (с нормализацией к цифрам — можно вводить ИНН с пробелами/дефисами или только цифры). **Фильтры:** расширенный блок (признаки, план, сегмент, склад, география, ответственные, рабочий режим) сворачивается; строка поиска и быстрые статусы остаются на виду. На узком экране по умолчанию блок свёрнут, на широком — развёрнут при первом визите; состояние «свёрнуто» хранится в `localStorage` (`tandoor-dealer-base-filters-collapsed-v1`). **Витрина дилеров** — единственный основной режим списка; переключатель плотности: **Крупно** / **Компактно** / **Список** / **Таблица** (`data-testid`: `section-dealer-showcase-mode-toolbar`, `button-dealer-showcase-density-large`, `button-dealer-showcase-density-compact`, `button-dealer-showcase-density-list`, `button-dealer-showcase-density-table`). Ключ `localStorage`: `tandoor-dealer-showcase-density-v1` (`large` \| `compact` \| `list` \| `table`). Без сохранённого значения: на узком экране (меньше 768px) по умолчанию **Компактно**, на десктопе — **Крупно**. Легаси-ключ `tandoor-dealer-base-view-mode-v1` при отсутствии нового **однократно** мигрирует в плотность: `dealer_showcase`, `cards`, пусто и неизвестные → **Крупно** (на узком при миграции — **Компактно**); `compact` / `list` / `table` сохраняются; после записи старый ключ удаляется. На узком экране **Таблица** рендерится как **Список**. Крупный вид — сетка карточек «партнёр / сеть»: клиент как дилер с бейджами и контактами (tel / «Написать» → wa.me / mailto), внутри карточки — торговые точки как филиалы со статусом витрины из `tradePointShowcaseActualizationById` (`deriveShowcaseBucket`, дефицит матрицы, порталы), теми же фильтрами, что и остальные виды; при массовом удалении — тот же красный селектор. **Геофильтры** (регион, район, населённый пункт): значения строятся из полей `city` и адреса строки без внешней геокодировки (`lib/dealer-base-geo-parse.ts`), эвристический разбор строки адреса; фильтрация подключена в `applyDealerBasePickerFilters`.
