...
import type { ClientBaseOverview } from "@/lib/client-base-overview-1c-api";
import { fetchClientBaseClientsList1c } from "@/lib/client-base-overview-1c-api";
...
const MODE_LS_KEY = "tandoor-clients-1c-management-mode-v1";
const OPEN_ROPS_LS_KEY = "tandoor-clients-1c-management-open-rops-v1";
...
export function ClientsOneCManagementCockpit({
  rows,
  profile,
  orgTeamCtx,
  overview,
  ...
  const tradePointsOverviewQ = useQuery({
    queryKey: ["trade-points-overview-1c"],
    queryFn: fetchTradePointsOverview1c,
    staleTime: 30_000,
  });
  const clientsListQ = useQuery({
    queryKey: ["client-base-clients-list-1c"],
    queryFn: () => fetchClientBaseClientsList1c({}),
    enabled:
      Boolean(overview) &&
  ...
  const managerDetailQ = useQuery({
    queryKey: ["trade-points-manager-detail-1c", selectedManagerApiUserId],
    queryFn: () => fetchTradePointsManagerDetail1c(selectedManagerApiUserId as string),
  ...
  if (overview) {
    const isKpiDetail = detail?.kind === "kpi-clients" || detail?.kind === "kpi-trade-points";
    return (
      <div className="min-w-0 max-w-full space-y-6 overflow-x-hidden pb-28 sm:pb-10" data-testid="page-clients-1c-overview">
        <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            <h1 className="text-2xl font-semibold tracking-tight text-foreground sm:text-3xl">Клиентская база 1С</h1>
            <p className="mt-1 text-sm text-muted-foreground">Управленческий обзор клиентской базы 1С.</p>
          </div>
          <div className="flex w-full shrink-0 flex-col gap-2 sm:w-auto sm:flex-row sm:justify-end">
            <Button variant="outline" size="sm" className="min-h-10 w-full sm:w-auto" asChild>
              <Link href={buildHashPath("/clients-1c")} data-testid="button-clients-1c-flat-list">
                Плоский список
              </Link>
            </Button>
            <Button variant="outline" size="sm" className="min-h-10 w-full sm:w-auto" asChild>
  ...
  return (
    <div className="min-w-0 max-w-full space-y-6 overflow-x-hidden pb-28 sm:pb-10" data-testid="page-clients-1c-overview">
      <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          <h1 className="text-2xl font-semibold tracking-tight text-[#222631] sm:text-3xl">Клиентская база 1С</h1>
          <p className="mt-1 text-sm text-[#8F96B0]">
            Управленческий обзор клиентской базы 1С (данные из обмена).
          </p>
        </div>
        <div className="flex w-full shrink-0 flex-col gap-2 sm:w-auto sm:flex-row sm:justify-end">
