...
import { formatCurrency, formatDate, toTitleWords } from "@/lib/format";
  ...
  if (!Number.isFinite(orderId)) {
    return (
      <Alert variant="destructive" data-testid="order-detail-invalid">
        ...
        <AlertDescription>The requested order id is not valid.</AlertDescription>
      </Alert>
    );
  }
  ...
  if (
    ...
    return (
      <Alert variant="destructive" data-testid="order-detail-error">
        <AlertCircle className="h-4 w-4" />
        <AlertTitle>Unable to load order data</AlertTitle>
        <AlertDescription>Please retry in a few moments.</AlertDescription>
      </Alert>
    );
  }
  ...
  if (!order) {
    return (
      <Alert data-testid="order-detail-empty">
        ...
        <AlertDescription>This order does not exist in the current demo dataset.</AlertDescription>
      </Alert>
    );
  }
    ...
    <div className="space-y-6" data-testid="order-detail-page">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Created on {formatDate(order.createdAt)} by{" "}
            {createdBy ? `${createdBy.firstName} ${createdBy.lastName}` : "Unknown user"}
          </p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Dealer info, status and execution timeline.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="grid gap-3 md:grid-cols-4">
            <div className="rounded-xl border border-border bg-muted/30 p-4">
              <p className="text-xs uppercase tracking-wide text-muted-foreground">Dealer</p>
              <p className="mt-1 font-medium">{dealerOrganization?.name ?? "Unknown dealer"}</p>
            </div>
            <div className="rounded-xl border border-border bg-muted/30 p-4">
              <p className="text-xs uppercase tracking-wide text-muted-foreground">Requested delivery</p>
              <p className="mt-1 font-medium">
                {order.requestedDeliveryDate ? formatDate(order.requestedDeliveryDate) : "Not set"}
              </p>
            </div>
            <div className="rounded-xl border border-border bg-muted/30 p-4">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Invoices, contracts and shipment files.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-2">
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs capitalize text-muted-foreground">{toTitleWords(document.type)}</p>
                  </div>
                </div>
          ...
          <Separator />
          <p className="text-xs text-muted-foreground">Document workflow actions will be implemented in a future MVP step.</p>
        </CardContent>
      </Card>
    </div>
  );
