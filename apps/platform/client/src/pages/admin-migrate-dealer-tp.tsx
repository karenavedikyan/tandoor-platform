/**
 * Admin: синхронное применение DDL оверрайдов дилера и ТТ к Neon и Yandex (Промт 113).
 */
...
export default function AdminMigrateDealerTpPage() {
  ...
  async function runMigrate() {
    ...
    setResult(null);
    try {
      const res = await fetch("/api/admin/migrate-dealer-tp", {
        method: "POST",
        credentials: "include",
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-5 pb-24 p-4 sm:p-6" data-testid="page-admin-migrate-dealer-tp">
      <div>
        <h1 className="text-2xl font-semibold text-[#222631]">Миграции оверрайдов дилера и ТТ</h1>
        <p className="mt-1 text-sm text-[#8F96B0]">
          Таблицы dealer_overrides, trade_point_overrides и связанные. Neon — с Vercel; Yandex — через прокси или вручную.
        </p>
      </div>
