...
import {
  addTradePointContact,
  canEditClientContacts,
  CLIENT_CONTACTS_EVENT,
  getTradePointContacts,
  isClientContactActive,
  requestDeleteTradePointContact,
  setPrimaryTradePointContact,
...
export function TradePointContactsSection({ row, tradePoint, profile }: Props) {
  ...
  const contacts = useMemo(() => getTradePointContacts(row, tradePoint), [row, tradePoint, tick]);
  ...
  const onSaveForm = useCallback(() => {
    ...
    if (editingId) {
      ...
      addTradePointContact(
        ...
        profile,
        { tradePointDisplayName: tradePoint.name },
      );
    }
    setTick((n) => n + 1);
    setEditOpen(false);
  }, [draft, editingId, canEdit, row.id, tradePoint.id, tradePoint.name, profile]);

  const renderContactBlock = (c: ClientContact, isPrimaryRow: boolean) => (
    <div
      key={c.id}
      data-testid={`row-trade-point-contact-${c.id}`}
      className={cn(
    ...
    >
      <div className="flex min-w-0 flex-wrap items-center gap-2">
        <p className="min-w-0 shrink font-semibold text-foreground" data-testid={`text-trade-point-contact-name-${c.id}`}>
          {c.fullName}
        </p>
        {c.isPrimary ? (
          <Badge variant="outline" className="text-[10px] shrink-0" data-testid={`badge-trade-point-contact-primary-${c.id}`}>
            Основной
          </Badge>
        ) : null}
        {c.isActual ? (
          <Badge variant="secondary" className="text-[10px] shrink-0" data-testid={`badge-trade-point-contact-actual-${c.id}`}>
            Актуальный
          </Badge>
        ) : null}
      </div>
      ...
      {isFilled(c.phone) ? (
        <p className="min-w-0 break-words">
          <a href={telHref(c.phone!)} className="font-medium text-primary underline-offset-2 hover:underline" data-testid={`link-trade-point-contact-phone-${c.id}`}>
            {c.phone}
          </a>
        </p>
      ) : null}
      {isFilled(c.whatsapp) ? (
        <p className="min-w-0 break-words">
          <a
            ...
            data-testid={`link-trade-point-contact-whatsapp-${c.id}`}
          >
            WhatsApp: {c.whatsapp}
          </a>
        </p>
      ...
      {isFilled(c.telegram) ? (
        <p className="min-w-0 break-words">
          <a
            ...
            rel="noreferrer"
            className="font-medium text-primary underline-offset-2 hover:underline"
            data-testid={`link-trade-point-contact-telegram-${c.id}`}
          >
            Telegram: {c.telegram}
          </a>
        </p>
      ...
      {isFilled(c.email) ? (
        <p className="min-w-0 break-words">
          <a href={mailHref(c.email!)} className="font-medium text-primary underline-offset-2 hover:underline" data-testid={`link-trade-point-contact-email-${c.id}`}>
            {c.email}
          </a>
        </p>
      ) : null}
      ...
      {canEdit ? (
        <div className="flex flex-wrap gap-1.5 pt-1">
          <Button
            ...
            data-testid={`button-trade-point-contact-edit-${c.id}`}
            onClick={() => openEdit(c)}
          >
            Изменить
          </Button>
          {!c.isPrimary ? (
            <Button
              ...
              size="sm"
              className="h-8 px-2 text-[11px] font-semibold"
              data-testid={`button-trade-point-contact-set-primary-${c.id}`}
              onClick={() => {
                setPrimaryTradePointContact(row.id, tradePoint.id, c.id, profile);
                setTick((n) => n + 1);
              }}
            >
              Сделать основным
          ...
          <Button
            ...
            size="sm"
            className="h-8 px-2 text-[11px] font-semibold"
            data-testid={`button-trade-point-contact-delete-request-${c.id}`}
            onClick={() => {
  ...
  return (
    <section data-testid="section-trade-point-contacts" className="space-y-3">
      <div className="space-y-1">
        <h2 className="text-base font-semibold tracking-tight text-foreground sm:text-lg">Контакты торговой точки</h2>
        <p className="text-sm text-muted-foreground">Контакты точки хранятся отдельно от карточки дилера.</p>
      </div>
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              {canEdit ? (
                <Button
                  ...
                  data-testid="button-trade-point-contact-add"
                  onClick={openAdd}
                >
                  Добавить контакт
                </Button>
            ...
            <div className="rounded-lg border border-dashed border-border/80 bg-muted/15 px-3 py-3 text-sm">
              ...
              {canEdit ? (
                <Button
                  ...
                  variant="default"
                  size="sm"
                  className="mt-3 min-h-9 w-full font-semibold sm:w-auto"
                  data-testid="button-trade-point-contact-add"
                  onClick={openAdd}
                >
                  Добавить контакт
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-md" data-testid="dialog-trade-point-contact-edit">
          <DialogHeader>
          ...
          <div className="space-y-3 py-1">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                data-testid="input-trade-point-contact-name"
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Input
                ...
                data-testid="input-trade-point-contact-role"
              />
            </div>
            <div className="space-y-1.5">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  data-testid="input-trade-point-contact-whatsapp"
                />
              </div>
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  data-testid="input-trade-point-contact-telegram"
                />
              </div>
            </div>
            <div className="space-y-1.5">
              ...
              <Input
                ...
                data-testid="input-trade-point-contact-email"
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Textarea
                ...
                data-testid="textarea-trade-point-contact-comment"
              />
            </div>
            <label className="flex cursor-pointer items-center gap-2 text-xs">
              <Checkbox
                ...
                data-testid="checkbox-trade-point-contact-primary"
              />
            ...
            <label className="flex cursor-pointer items-center gap-2 text-xs">
              <Checkbox
                ...
                data-testid="checkbox-trade-point-contact-actual"
              />
              <span>Актуален</span>
            </label>
          </div>
          <DialogFooter>
            <Button type="button" className="min-h-10 w-full font-semibold sm:w-auto" data-testid="button-trade-point-contact-save" onClick={onSaveForm}>
              Сохранить
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <Textarea
            ...
            data-testid="textarea-trade-point-contact-delete-reason"
          />
          <DialogFooter>
            <Button
              type="button"
              className="min-h-10 font-semibold"
              data-testid="button-trade-point-contact-delete-confirm"
              onClick={() => {
                if (deleteContactId) {
                  requestDeleteTradePointContact(row.id, tradePoint.id, deleteContactId, deleteReason, profile);
                  setTick((n) => n + 1);
                }
                setDeleteOpen(false);
              }}
