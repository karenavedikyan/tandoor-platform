/**
 * PILOT ONLY. Это временная mock-авторизация. Будет полностью удалена в PR `auth-client-switch-cd7c`, после того как заработают серверные сессии. Не использовать как механизм безопасности. См. `docs/auth-access-foundation.md`.
 *
