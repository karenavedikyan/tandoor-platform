...
Object.defineProperty(globalThis, "fetch", {
  value: async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url.includes("/api/showcase-matrix/scope-all")) scopeAllFetchCalls += 1;
    else if (url.includes("/api/showcase-matrix/scope")) scopeFetchCalls += 1;
    if (fetchShouldFail) {
      throw new Error("network down");
    }
    if (url.includes("/api/showcase-matrix/scope-all") && init?.method === "POST") {
      const entries: ShowcaseMatrixEntryDto[] = [
        {
          id: "e-tp1",
          dealerId: "d1",
          tradePointId: "tp1",
      ...
      return {
        ok: true,
        async json() {
          return { success: true, entries, tradePointIds: ["tp1"] };
        },
      };
    }
...
assert.equal(scopeAllFetchCalls, 1);
assert.equal(scopeFetchCalls, 0);
...
fetchShouldFail = false;
scopeFetchCalls = 0;
scopeAllFetchCalls = 0;
fetchCalls = 0;
resetDistributionAnalyticsStoreForTests();
await ensureDistributionAnalyticsData({ scope: tpScope, force: true });
const beforeThrottleAll = scopeAllFetchCalls;
const beforeThrottleScope = scopeFetchCalls;
await ensureDistributionAnalyticsData({ scope: tpScope });
assert.equal(scopeAllFetchCalls, beforeThrottleAll, "throttled second call should not fetch scope-all");
assert.equal(scopeFetchCalls, beforeThrottleScope, "throttled second call should not fetch scope");
