...
import { computeDbScopeForUser, computeDealerKpiTotalsForActiveDealers, type DbScopeTotals } from "./db-scope-formula.js";
import {
...
import type { TeamScopeMember, TeamScopePayload, TeamTotals } from "./dealers-scope-types.js";
...
function teamTotalsFromDbScope(totals: DbScopeTotals): TeamTotals {
  return {
    active_dealers: totals.active_dealers,
    active_trade_points: totals.active_trade_points,
    trashed_dealers: totals.trashed_dealers,
    trashed_trade_points: totals.trashed_trade_points,
    tp_status_active: totals.tp_status_active,
    tp_status_potential: totals.tp_status_potential,
    tp_status_attention: totals.tp_status_attention,
    dealer_no_status: totals.dealer_no_status,
    avg_distribution: totals.avg_distribution,
  };
}
...
export async function buildTeamScopePayload(
  ...
  let teamTotals: TeamTotals;
  if (team.rop_user_id) {
    const ropScope = await computeDbScopeForUser(pool, team.rop_user_id, "rop");
    teamTotals = teamTotalsFromDbScope(ropScope.totals);
  } else {
