/**
 * Общая логика вызовов DaData для Vercel-функций (`api/dadata/*`) и Express (`server/dadata-routes.ts`).
 * Только fetch + разбор JSON, без зависимостей от @vercel/node.
 */
...
export type DadataPartyLookupItem = {
  id: string;
  name: string;
  inn: string;
  kpp?: string;
...
export function isDadataAddressSuggestEnabled(): boolean {
...
export function isDadataPartyLookupEnabled(): boolean {
  const key = getDadataApiKey();
  if (!key) return false;
...
export function parseAddressSuggestBody(
  raw: unknown,
...
export function parsePartyFindBody(raw: unknown): { ok: true; inn: string } | { ok: false; message: string } {
  const body = raw != null && typeof raw === "object" && !Array.isArray(raw) ? (raw as Record<string, unknown>) : {};
...
function mapAddressSuggestion(s: Record<string, unknown>): DadataAddressSuggestItem {
  const data = s.data != null && typeof s.data === "object" && !Array.isArray(s.data) ? (s.data as Record<string, unknown>) : {};
...
export async function fetchDadataPartyByInn(
  inn: string,
  apiKey: string,
...
export function readJsonBodyFromUnknown(body: unknown, rawString?: string): unknown {
